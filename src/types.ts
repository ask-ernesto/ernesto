 ...
 * Works with external APIs, filesystem, etc.
 */
export interface ResourceNode<TMeta = any> {
    ...
    /**
     * OPTIONAL: Short summary for search results (recommended)
     * If not provided, content will be truncated for search result display
     */
    description?: string;

    /**
     * OPTIONAL: Source-specific metadata
     * Only put truly source-specific fields here
     * (taskId, suiteId, sha, etc - things not needed for core indexing)
     */
    metadata?: TMeta;
...
export interface PipelineConfig {
    ...
    /**
     * Access scopes for all resources from this pipeline
     *
     * These scopes are merged with domain-level scopes to determine final access control.
     * Scope inheritance: domain scopes + pipeline scopes = final resource scopes
     *
     * Defaults to [] (unrestricted) if not specified.
     *
     * @example
     * // Unrestricted resources (default)
     * scopes: []
     *
     * // Internal-only resources
     * scopes: ['internal']
     *
     * // Sensitive resources requiring specific permission
